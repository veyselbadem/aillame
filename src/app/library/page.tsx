      ...
      <div className="max-w-5xl mx-auto space-y-10 animate-fade-in">
        <header className="space-y-4">
          ...
          <p className="text-gray-400 text-lg max-w-3xl">
            Belgelerini burada analiz edebilirsin. Faz 4 RAG foundation; txt, markdown, json ve code/text ingestion için güvenli chunking ve attribution hazırlığı sağlar.
          </p>
        </header>

        <section className="grid gap-3 md:grid-cols-4">
          {[
            ['Document Ingestion', 'foundation ready'],
            ['Vector Memory', 'placeholder store'],
            ['PDF/DOC Parser', 'not supported here'],
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-4 space-y-6">
            <div className="glass-card p-8 rounded-[24px] border-white/5 space-y-6">
              ...
              <div className="p-4 bg-white/[0.02] rounded-2xl border border-white/5">
                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest leading-relaxed">
                  Yüklenen dokümanlar yerel olarak işlenir. Secret/token/key benzeri içerikler RAG hafızasına otomatik alınmamalıdır.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-8">
            {!doc && !loading && (
              <div className="glass-card h-[400px] rounded-[24px] border-white/5 border-dashed flex flex-col items-center justify-center text-center p-8 opacity-60">
                <FiFileText size={48} className="mb-4 text-gray-600" />
                <p className="text-gray-400 font-medium">Henüz bir doküman seçilmedi.</p>
                <p className="text-xs text-gray-500 mt-2">Analiz sonuçları ve attribution preview burada görünecek.</p>
              </div>
            )}

            {loading && (
              <div className="glass-card h-[400px] rounded-[24px] border-white/5 flex flex-col items-center justify-center space-y-6">
                <div className="w-12 h-12 border-[4px] border-indigo-500/10 border-t-indigo-500 rounded-full animate-spin" />
                <p className="text-indigo-300/40 text-[10px] font-black uppercase tracking-[0.4em]">Document Analysis...</p>
              </div>
            )}
            ...
            {doc && !loading && (
              <div className="space-y-6 animate-slide-up">
                ...
                {analysis && (
                  <div className="glass-card p-8 rounded-[24px] border-indigo-500/20 bg-indigo-500/[0.02] space-y-6">
                    ...
                    <p className="text-xs text-gray-500">Attribution preview: Bu analiz henüz kalıcı vector memory kaynağına bağlanmadı.</p>
                  </div>
                )}
              </div>
            )}
