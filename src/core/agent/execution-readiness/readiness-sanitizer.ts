...
/**
 * Sanitizes readiness request data by masking sensitive patterns and limiting length.
 */
export function sanitizeReadinessInput(text: string | undefined, maxLength: number): string {
  if (!text) return "";
...
/**
 * Detects intent that looks like commands or file writes in text.
 */
export function detectReadinessIntents(text: string | undefined): { hasCommandIntent: boolean; hasWriteIntent: boolean } {
